...
const downloader = require('electron-dl').download
...
app.on('ready', function() {
  ...
  parent.respondTo('downloadImage', function(url, pathname, done) {
    const directory = apath.dirname(pathname)
    const filename = apath.basename(pathname)
    return downloader(win, url, {
      directory,
      filename 
    }).then(() => {
      done();
